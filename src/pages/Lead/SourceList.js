...
function SourceList() {
  ...
  const sectionName = lang("Source");
  const routeName = "source";

  const api = {
    addEdit: apiPath.addEditSource,
    list: apiPath.listSource,
  };
  ...
  return (
    <>
      <SectionWrapper
        cardHeading={lang("Source") + " " + lang("List")}
        extra={
          <>
            <div className="w-100 text-head_right_cont">
              <div className="pageHeadingSearch">
                <Input.Search
                  ...
                  placeholder={lang("Search by source name")}
                  onChange={onSearch}
                  allowClear
                />
              </div>
      ...
      {showDelete && (
        <DeleteModal
          title={lang("Delete Source")}
          subtitle={lang(`Are you sure you want to Delete this source?`)}
          show={showDelete}
          hide={() => {
            setShowDelete(false);
            setSelected();
